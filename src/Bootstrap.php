...
add_action(
	'init',
	function () {
		load_plugin_textdomain( 'git-updater-gitea' );
	}
);
...
class Bootstrap {
	...
	/**
	 * Add hosts authorized to receive Gitea credentials.
	 *
	...
	public function set_credential_hosts( $hosts, $installed_apis, $repos ) {
		...
		if ( ! empty( $options['gitea_server'] ) ) {
			$host = wp_parse_url( $options['gitea_server'], PHP_URL_HOST );
			if ( ! empty( $host ) ) {
				$hosts['gitea'][] = (string) $host;
			}
		}

		foreach ( $repos as $repo ) {
			if ( ! isset( $repo->git ) || 'gitea' !== $repo->git ) {
				continue;
			}
	...
	public function set_remote_install_data( $install, $headers ) {
		if ( 'gitea' === $install['git_updater_api'] ) {
			...
			if ( ! $api->is_allowed_credential_host( (string) ( $install['download_link'] ?? '' ), 'gitea' ) ) {
				$install['error'] = new WP_Error(
					'gu_install_host_not_allowed',
					sprintf(
						/* translators: %s: hostname of the install source. */
						esc_html__( 'The install source %s is not an allowed host.', 'git-updater-gitea' ),
